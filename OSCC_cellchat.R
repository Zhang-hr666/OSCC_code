...
library(patchwork)
## 导入Seurat对象
seu <- qs::qread("data/OSCC_annotated_STref.qs")
table(seu$combined)
####改一下细胞簇名
library(dplyr)
# 确保 'combined' 列是字符变量
...
Idents(seu) <- "combined"
table(seu$combined)
seu <- subset(seu, downsample = 350)
# # 将 'combined' 列转换回因子变量，并指定期望的水平
# seu$combined <- factor(seu$combined, levels = c("B/Plasma", "DC", "Endothelial", "Epithelial01", "Epithelial02",
#                                                 "Epithelial03", "Epithelial04", "Epithelial05", "Epithelial06", "Epithelial07",
#                                                 "F00", "F01", "F02", "F03", "F04", "F06", "F07", "F08", "F09", "F10",
#                                                 "Macrophages", "Mast", "Myocytes", "OSCC_Normal", "T/NK"))
# # 检查更新后的水平
# table(seu$combined)
# # seu <- subset(seu, celltype %in% c("Mono/Mk Doublets", "Eryth"), invert = TRUE)
# # seu <- subset(seu, combined %in% c("Epithelial01", "Epithelial02"
# #                                    , "Epithelial03", "Epithelial04"
# #                                    , "Epithelial05", "Epithelial06"
# #                                    , "Epithelial07"))##上皮
# seu <- subset(seu, combined %in% c( "F00", "F01", "F02", "F03", "F04", "F06", "F07", "F08", "F09", "F10",
#                                     "OSCC_Normal","Epithelial01", "Epithelial02",
#                                     "Epithelial03", "Epithelial04", "Epithelial05",
#                                     "Epithelial06", "Epithelial07"))
# DimPlot(seu, group.by = "combined", reduction = "harmony_umap", label = T)
#######清洗完了，只剩下了上皮和成纤维
####  02单个cellchat对象的构建  ####
...
cellchat <- createCellChat(object = seu, 
                           group.by = "combined", assay = "RNA")
# cellchat <- addMeta(cellchat, meta = meta)
cellchat <- setIdent(cellchat, ident.use = "combined") # set "labels" as default cell identity
...
##  将细胞簇分组为不同的细胞类型
# Chord diagram
group.cellType <- c("B/Plasma","DC","Endothelial" ,"Epithelial01", "Epithelial02", "Epithelial03",
 "Epithelial04","Epithelial05","Epithelial06","Epithelial07" ,"F00","F01"         
,"F02"    , "F03" , "F04" , "F06"  ,  "F07" , "F08"         
  ,"F09"   , "F10"  ,  "Macrophages","Mast"  , "Myocytes", "OSCC_Normal" 
 , "T/NK") # grouping cell clusters into fibroblast, DC and TC cells
...
# Signaling role analysis on the cell-cell communication networks of interest
ht <- netAnalysis_signalingRole_heatmap(cellchat, signaling = c("ANGPTL"))
ht
ht1 <- netAnalysis_signalingRole_heatmap(cellchat, pattern = "outgoing", signaling = c("ANGPTL"))
ht2 <- netAnalysis_signalingRole_heatmap(cellchat, pattern = "incoming", signaling = c("ANGPTL"))
ht1 + ht2
...
##  构建对象
cellchat.NL <- Cellchat_proceeding(object = humanSkin_ser_list$NL, 
                                   group.by = "labels", assay = "RNA",
                                   workers = 10)
cellchat.LS <- Cellchat_proceeding(object = humanSkin_ser_list$LS, 
                                   group.by = "labels", assay = "RNA",
                                   workers = 10)
...
save(cellchat.NL,cellchat.LS,object.list,file = './output/05CellChat_list.Rdata')
